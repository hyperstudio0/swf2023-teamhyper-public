...
export default function WhitelistNewEditForm({currentWhiteList}: Props) {
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            ...
            const response =
                await Swagger.api.scPostWhiteList({
                    did: data.did
                })
            console.log(response.data, 'response');
            reset();
            enqueueSnackbar("화이트-리스트 등록이 완료되었습니다.");
        ...
        <>
            ...
            <Grid xs={12} md={8} sx={{display: 'flex', alignItems: 'center'}}>
                <LoadingButton type="submit" variant="contained" size="large" loading={isSubmitting}>
                    {!currentWhiteList ? '화이트리스트 등록하기' : 'Save Changes'}
                </LoadingButton>
            </Grid>
        </>
    );
